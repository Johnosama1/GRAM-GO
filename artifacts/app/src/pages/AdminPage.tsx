...
export default function AdminPage() {
  ...
  const [newPromoCode, setNewPromoCode] = useState({ code: "", rewardType: "GRAM", rewardAmount: "10", maxUses: "100", expiresAt: "" });
  ...
  const [selectedComboItems, setSelectedComboItems] = useState<number[]>([
    1, 2, 3,
  ]);
  ...
  const [selectedUserDetail, setSelectedUserDetail] =
    useState<UserDetailResult | null>(null);
  ...
  const [maintenanceText, setMaintenanceText] = useState(
    "البوت حالياً في وضع الصيانة للتطوير والتحديث. سنعود للعمل قريباً!",
  );
  ...
  // User Message Modal
  const [userMsgModal, setUserMsgModal] = useState<{
    open: boolean;
    userId?: number;
    isWarning?: boolean;
  }>({ open: false });
    ...
    try {
      const [
        ...
        api
          ...
          .catch(() => ({ 1: 2, 2: 3, 3: 4, 4: 5, 6: 7, 7: 10 })),
      ...
      if (setts) {
        ...
        if (setts["monetag_zone_id"])
          setMonetagZoneId(setts["monetag_zone_id"]);
        if (setts["ad_reward_rush"]) setAdRewardRush(setts["ad_reward_rush"]);
        if (setts["ad_daily_watch_limit"])
          setAdDailyWatchLimit(setts["ad_daily_watch_limit"]);
        if (setts["maintenance_message"])
          setMaintenanceText(setts["maintenance_message"]);
        if (setts["deposit_wallet_address"])
          setDepositWalletAddress(setts["deposit_wallet_address"]);
      }
  ...
  const handleCreatePromoCode = async () => {
    try {
      const res = await api.adminCreatePromoCode(newPromoCode);
      if (res) {
    ...
    const rawUser =
      ...
      (
        window as unknown as {
          Telegram?: {
            WebApp?: { initDataUnsafe?: { bot_username?: string } };
          };
        }
      )?.Telegram?.WebApp?.initDataUnsafe?.bot_username ||
      "GRAM_GO_BOT";
  ...
  const handleBroadcast = async () => {
    if (!broadcastText.trim())
      return showToast("يرجى كتابة نص الرسالة أولاً", "err");
    try {
      ...
      showToast(
        `تم إرسال الرسالة إلى ${res.totalUsers || "جميع"} المستخدمين بنجاح 📢`,
      );
      setBroadcastText("");
    } catch (err: unknown) {
  ...
  const handleAddAdmin = async () => {
    ...
    if (!idNum || isNaN(idNum))
      return showToast("يرجى إدخال Telegram ID صحيح", "err");
    try {
      await api.adminAddAdmin({
        ...
        role: "moderator",
        permissions:
          newAdminPerms.length > 0 ? newAdminPerms : ["canViewStats"],
      });
  ...
  const handleWithdrawalAction = async (
    ...
    if (action === "reject") {
      const input = prompt(
        "يرجى كتابة سبب رفض السحب (سيتم إرجاع الرصيد للمستخدم):",
        "بيانات المحفظة غير صحيحة",
      );
    ...
    try {
      ...
      showToast(
        action === "approve"
          ? "تم قبول وتأكيد السحب بنجاح ✅"
          : "تم رفض السحب وإرجاع الرصيد للمستخدم 🔄",
      );
      setWithdrawals((prev) =>
        prev.map((w) =>
          w.id === id
  ...
  const handleSaveDepositWallet = async () => {
    if (!depositWalletAddress.trim())
      return showToast("يرجى كتابة عنوان المحفظة", "err");
    try {
  ...
  const handleResetBalances = async (type: "coins" | "gram") => {
    try {
      if (type === "coins") {
        ...
        showToast(
          `تم تصفير رصيد الـ Rush/Coins لـ ${res.affectedUsers || 0} مستخدم بنجاح ♻️`,
        );
      } else {
        const res = await api.adminResetGramBalances("CONFIRM_RESET_GRAM");
        showToast(
          `تم تصفير رصيد الـ GRAM لـ ${res.affectedUsers || 0} مستخدم بنجاح ♻️`,
        );
      }
      setResetModal(null);
    } catch {
  ...
  const handleAdjustBalance = async () => {
    ...
    try {
      ...
      showToast(
        `تم ${balanceAdjustForm.type === "add" ? "إضافة" : balanceAdjustForm.type === "deduct" ? "خصم" : "تصحيح"} الرصيد بنجاح 💰`,
      );
  ...
  const handleSendMessageToUser = async () => {
    if (!userMsgModal.userId || !userMsgText.trim())
      return showToast("يرجى كتابة نص الرسالة", "err");
    try {
  ...
  const handleBanUser = async (id: number, ban: boolean) => {
    try {
      if (ban) {
        const reason =
          prompt("سبب الحظر:", "مخالفة الشروط والاحتيال") || undefined;
  ...
  const handleToggleWithdrawalBan = async (
    id: number,
  ...
  const handleDeleteUserAccount = async () => {
    ...
    try {
      ...
      setUserSearchResults((prev) =>
        prev.filter((u) => u.id !== deleteUserModal.userId),
      );
    } catch {
    ...
    >
      ...
      {msg && (
        <div
          style={{
            ...
            background:
              msg.type === "ok"
                ? "linear-gradient(135deg, #0FD37C, #059669)"
                : "linear-gradient(135deg, #E5484D, #b91c1c)",
            color: "#fff",
            padding: "12px 18px",
            borderRadius: 16,
            fontWeight: 800,
      ...
      >
        {[
          ...
          {
            id: "tasks" as const,
            label: "المهام والمكافآت",
            icon: Gift,
            badge: null,
          },
          {
            id: "ads" as const,
            label: "شبكة الإعلانات",
            icon: PlayCircle,
            badge: null,
          },
          {
            ...
            icon: Shield,
            badge: (autoBannedList || []).length || null,
          },
        ].map((tab) => {
          ...
          return (
            <button
              ...
              style={{
                ...
                flexShrink: 0,
                background: isActive
                  ? "linear-gradient(135deg, #0FA0D6, #11ABEC)"
                  : "#101418",
                border: isActive
                  ? "1px solid #11ABEC"
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          ...
          >
            ...
            >
              ...
              >
                <div
                  style={{ fontSize: 18, fontWeight: 900, color: "#E5484D" }}
                >
              ...
              <div
                style={{
                  background: "#1A212D",
                  border: "1px solid rgba(15, 211, 124, 0.4)",
                  borderRadius: 14,
                  padding: "12px 6px",
                  textAlign: "center",
                }}
              >
                <div
                  style={{ fontSize: 18, fontWeight: 900, color: "#0FD37C" }}
                >
              ...
              <div
                style={{
                  background: "#1A212D",
                  border: "1px solid rgba(17, 171, 236, 0.4)",
                  borderRadius: 14,
                  padding: "12px 6px",
                  textAlign: "center",
                }}
              >
                <div
                  style={{ fontSize: 18, fontWeight: 900, color: "#11ABEC" }}
                >
                  {stats?.totalUsers ?? 0}
                </div>
                <div
            ...
            >
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <strong style={{ color: "#0FD37C" }}>
                  {parseFloat(stats?.totalTonWithdrawn || "0").toFixed(4)} TON
                </strong>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ color: "#8A8F98" }}>
                  إجمالي عملات GO المتداولة:
                </span>
                <strong style={{ color: "#11ABEC" }}>
                  {parseFloat(stats?.totalGo || "0").toLocaleString()} GO
                </strong>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ color: "#8A8F98" }}>
                  إجمالي عملات GRAM المعدنة:
                </span>
                <strong style={{ color: "#0FD37C" }}>
                  {parseFloat(stats?.totalGram || "0").toFixed(4)} GRAM
                </strong>
              </div>
            </div>
          ...
          >
            ...
            >
              ...
              />
              <label
                htmlFor="pin_broadcast"
                style={{ fontSize: 12, color: "#E4E6EB", cursor: "pointer" }}
              >
            ...
            >
              <Send size={15} />
              <span>
                {broadcastSending ? "جاري الإرسال..." : "📢 إرسال للجميع الآن"}
              </span>
            </button>
            ...
            <div
              style={{
                background:
                  "linear-gradient(135deg, rgba(168, 85, 247, 0.12), rgba(124, 58, 237, 0.06))",
            ...
            >
              ...
              >
                ...
                <span
                  style={{ fontSize: 13, fontWeight: 900, color: "#d8b4fe" }}
                >
              ...
              >
                لو محتاج تستخدم{" "}
                <b>الإيموجي المميز (Telegram Premium Custom Emojis)</b> تحديدًا
                في رسالة البث، تليجرام بيتطلب كتابة الرسالة داخل شات البوت
                الفعلي (مش من داخل الميني آب) عشان يقدر يحفظ بيانات الإيموجي
                بشكل صحيح — ده قيد من تليجرام نفسه مش من التطبيق. غير كده،
                استخدم الصندوق اللي فوق وكله هيفضل جوه الميني آب.
              </p>
              <button
          ...
          >
            ...
            >
              <div>
                ...
                >
                  {isMaintenance
                    ? "🔴 وضع الصيانة مفعّل (البوت مقفل للمستخدمين)"
                    : "🟢 البوت مفعّل ويعمل لجميع المستخدمين"}
                </div>
              </div>
            ...
            <button
              onClick={() =>
                handleUpdateSetting("maintenance_message", maintenanceText)
              }
              style={{
                width: "100%",
                height: 42,
          ...
          >
            ...
            >
              <span style={{ fontSize: 11, fontWeight: 800, color: "#11ABEC" }}>
              ...
              >
                {newAdminPerms.length === ALL_PERMISSIONS.length
                  ? "إلغاء التحديد"
                  : "تحديد الكل"}
              </button>
            </div>
                  ...
                  >
                    <input
                      ...
                      onChange={() => {
                        if (checked) {
                          setNewAdminPerms(
                            newAdminPerms.filter((x) => x !== p.key),
                          );
                        } else {
              ...
              {admins.map((adm) => (
                ...
                >
                  <div>
                    ...
                    <div
                      style={{ fontSize: 9, color: "#11ABEC", marginTop: 2 }}
                    >
                      {adm.permissions?.includes("*")
                        ? "كل الصلاحيات"
                        : `${adm.permissions?.length || 0} صلاحيات`}
                    </div>
                  </div>
                  <button
              ...
              {auditLogs.length === 0 ? (
                ...
                auditLogs.map((log) => (
                  ...
                  >
                    ...
                    <div
                      style={{ fontSize: 10, color: "#8A8F98", marginTop: 2 }}
                    >
                      مشرف: #{log.adminId}{" "}
                      {log.targetUserId ? `• مستهدف: #${log.targetUserId}` : ""}
                    </div>
                  </div>
                ))
              )}
                  ...
                  >
                    <div
                      style={{
                        width: `${c.percent}%`,
                        height: "100%",
                        background: "linear-gradient(90deg, #0FA0D6, #11ABEC)",
                      }}
                    />
                  </div>
                </div>
      ...
      {activeTab === "mining" && (
        <div style={{ display: "flex", flexDirection: "column" }}>
          ...
          >
            ...
            >
              نسبة التعدين اليومية من رصيد GO (الافتراضي 3.0% أي كل 1000 GO تنتج
              30 GRAM يومياً).
            </p>
            ...
            >
              ...
              />
              <span style={{ fontSize: 12, color: "#8A8F98", fontWeight: 800 }}>
                % / يوم
              </span>
            </div>
            ...
            >
              ...
              <strong style={{ color: "#11ABEC" }}>
                +
                {((1000 * (parseFloat(miningRateInput) || 3.0)) / 100).toFixed(
                  4,
                )}{" "}
                GRAM / يوم
              </strong>
          ...
          >
            ...
            >
              عند الإخفاء، لا يحتاج المستخدم للضغط على زر التعدين وتُحسب الأرباح
              تلقائياً وبشكل مستمر.
            </p>
            ...
            >
              <button
                onClick={() =>
                  handleUpdateSetting("start_miner_visible", "true")
                }
                style={{
                  background: startMinerVisible
                    ? "linear-gradient(135deg, #0FA0D6, #11ABEC)"
                    : "rgba(255,255,255,0.04)",
                  border: startMinerVisible
                    ? "1px solid #11ABEC"
                    : "1px solid rgba(255,255,255,0.08)",
                  borderRadius: 10,
                  padding: 12,
              ...
              <button
                onClick={() =>
                  handleUpdateSetting("start_miner_visible", "false")
                }
                style={{
                  background: !startMinerVisible
                    ? "linear-gradient(135deg, #0FA0D6, #11ABEC)"
      ...
      {activeTab === "finance" && (
        <div style={{ display: "flex", flexDirection: "column" }}>
                    ...
                    {w.status === "pending" && (
                      <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                        <button
                          ...
                          style={{
                            flex: 1,
                            background: "rgba(15,211,124,0.2)",
                            border: "1px solid #0FD37C",
                            borderRadius: 8,
                            padding: 8,
                            color: "#0FD37C",
                            fontWeight: 900,
                  ...
                  >
                    ...
                    >
                      <span style={{ color: "#fff", fontWeight: 700 }}>
                        #{d.userId} ({d.username ? `@${d.username}` : "مستخدم"})
                      </span>
                      <strong style={{ color: "#0FD37C" }}>
                        +{d.amount} {d.currency}
                      </strong>
            ...
            >
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setLimits({ ...limits, minWithdrawal: e.target.value })
                  }
                  style={{
                    width: "100%",
                    background: "#080b10",
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setLimits({ ...limits, maxWithdrawal: e.target.value })
                  }
                  style={{
                    width: "100%",
                    background: "#080b10",
            ...
            >
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setLimits({ ...limits, minDeposit: e.target.value })
                  }
                  style={{
                    width: "100%",
                    background: "#080b10",
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setLimits({ ...limits, maxDeposit: e.target.value })
                  }
                  style={{
                    width: "100%",
                    background: "#080b10",
          ...
          >
            ...
            >
              سيتم تصفير رصيد الـ Rush لكل مستخدمي البوت (يرجع 0). هذا الإجراء
              لا يمكن التراجع عنه.
            </p>
            <button
          ...
          >
            ...
            >
              سيتم تصفير رصيد الـ GRAM لكل مستخدمي البوت (يرجع 0). هذا الإجراء
              لا يمكن التراجع عنه.
            </p>
            <button
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                ...
                <input
                  type="number"
                  placeholder="Amount"
                  value={newPromoCode.rewardAmount}
                  onChange={(e) =>
                    setNewPromoCode({
                      ...newPromoCode,
                      rewardAmount: e.target.value,
                    ...
                    <div>
                      ...
                      >
                        <span
                          style={{
                            color:
                              p.isActive === "true" ? "#0FD37C" : "#EF4444",
                          }}
                        >
                          {p.isActive === "true" ? "ACTIVE" : "INACTIVE"}
                        </span>{" "}
                        • {p.rewardAmount} {p.rewardType} • {p.currentUses}/
                        {p.maxUses}
                      </div>
                    </div>
                    <button
          ...
          >
            ...
            >
              <span style={{ fontSize: 11, color: "#fff", fontWeight: 800 }}>
                حالة قسم الهدايا للمستخدمين
              </span>
              <button
                onClick={() =>
                  handleUpdateSetting(
                    "gifts_section_open",
                    giftsOpen ? "false" : "true",
                  )
                }
                style={{
                  background: giftsOpen ? "#0FD37C" : "rgba(255,255,255,0.1)",
                  color: giftsOpen ? "#000" : "#fff",
                  border: "none",
                  borderRadius: 6,
                  padding: "4px 10px",
                  fontSize: 11,
      ...
      {activeTab === "ads" && (
        <div style={{ display: "flex", flexDirection: "column" }}>
          ...
          >
            ...
            <div
              style={{
                background: "rgba(15,211,124,0.08)",
                border: "1px solid rgba(15,211,124,0.3)",
                borderRadius: 10,
                padding: 10,
                display: "flex",
                alignItems: "center",
            ...
            >
              {[
                {
                  key: "ad_start_mining",
                  label: "إعلان عند بدء دورة تعدين جديدة",
                },
                {
                  key: "ad_claim_mining",
                  label: "إعلان عند المطالبة بأرباح التعدين",
                },
                {
                  key: "ad_daily_combo",
                  label: "إعلان قبل كشف الكومبو اليومي",
                },
              ].map((item) => {
                ...
                return (
                  ...
                  >
                    <span style={{ fontSize: 10, color: "#E4E6EB" }}>
                      {item.label}
                    </span>
                    <input
                      type="checkbox"
                      checked={active}
                      onChange={() =>
                        handleUpdateSetting(item.key, active ? "false" : "true")
                      }
                      style={{ cursor: "pointer" }}
                    />
                  </div>
            ...
            >
              {[
                {
                  key: "ad_watch_earn",
                  label: "تفعيل مهمة مشاهدة الإعلانات والربح",
                },
          ...
          >
            ...
            >
              <div>
                <div style={{ fontSize: 11, fontWeight: 800, color: "#fff" }}>
                  حالة نظام مكافحة التعدد
                </div>
                <div style={{ fontSize: 9, color: "#8A8F98" }}>
                  فحص بصمة الجهاز تلقائياً
                </div>
              </div>
              <button
                onClick={() =>
                  handleUpdateSetting(
                    ...
                    isSecurityActive ? "false" : "true",
                  )
                }
                style={{
                  background: isSecurityActive
                    ? "rgba(15,211,124,0.2)"
                    : "rgba(229,72,77,0.2)",
                  border: isSecurityActive
                    ? "1px solid #0FD37C"
                    : "1px solid #E5484D",
                  color: isSecurityActive ? "#0FD37C" : "#E5484D",
                  padding: "4px 10px",
                  borderRadius: 6,
                  fontWeight: 900,
                  fontSize: 10,
            ...
            >
              {autoBannedList.length === 0 ? (
                ...
                >
                  لا توجد حسابات محظورة تلقائياً
                </div>
              ) : (
                autoBannedList.map((ab) => (
                  ...
                  >
                    <div>
                      ...
                      >
                        #{ab.userId} {ab.username ? `@${ab.username}` : ""}
                      </div>
                      <div style={{ fontSize: 9, color: "#8A8F98" }}>
                        بصمة: {ab.ipHash?.slice(0, 14) || "N/A"}
                      </div>
                    </div>
                    <button
                      onClick={async () => {
              ...
              >
                ...
                >
                  <div>
                    ...
                    >
                      {selectedUserDetail.user.firstName || "مستخدم"}{" "}
                  ...
                  <span
                    style={{
                      background: selectedUserDetail.isBanned
                        ? "#E5484D"
                        : "#0FD37C",
                      color: "#000",
                      fontSize: 9,
                ...
                >
                  <div>
                    ...
                    >
                      {parseFloat(
                        selectedUserDetail.user.goBalance || "0",
                      ).toFixed(2)}
                    </div>
                  </div>
                  <div>
                    ...
                    >
                      {parseFloat(
                        selectedUserDetail.user.gramBalance || "0",
                      ).toFixed(4)}
                    </div>
                  </div>
                </div>
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      background: selectedUserDetail.isBanned
                        ? "rgba(15,211,124,0.2)"
                        : "rgba(229,72,77,0.2)",
                      border: selectedUserDetail.isBanned
                        ? "1px solid #0FD37C"
                        : "1px solid #E5484D",
                      borderRadius: 8,
                      padding: 6,
                      color: selectedUserDetail.isBanned
                        ? "#0FD37C"
                        : "#E5484D",
                      fontWeight: 800,
                      fontSize: 10,
                  ...
                  <button
                    onClick={() =>
                      handleToggleWithdrawalBan(
                        selectedUserDetail.user.id,
                        selectedUserDetail.isWithdrawalBanned || false,
                      )
                    }
                    style={{
                  ...
                  >
                    {selectedUserDetail.isWithdrawalBanned
                      ? "السماح بالسحب 🔓"
                      : "قفل السحب 🔒"}
                  </button>
                  <button
                    onClick={() =>
                      handleToggleDepositBan(
                        selectedUserDetail.user.id,
                        selectedUserDetail.isDepositBanned || false,
                      )
                    }
                    style={{
                      background: "rgba(251,191,36,0.15)",
                  ...
                  >
                    {selectedUserDetail.isDepositBanned
                      ? "السماح بالإيداع 🔓"
                      : "منع الإيداع 🔒"}
                  </button>
                </div>
              ...
              {milestones.map((m) => (
                ...
                >
                  <div style={{ fontSize: 12, fontWeight: 900, color: "#fff" }}>
                    {m.requiredReferrals} دعوة
                  </div>
                  <div
                    style={{ fontSize: 11, color: "#11ABEC", fontWeight: 800 }}
                  >
          ...
          >
            ...
            >
              <div>
                ...
                <div
                  style={{ fontSize: 16, fontWeight: 900, color: "#0FD37C" }}
                >
                  0% آمن تماماً
                </div>
              </div>
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
              {(["GO", "Gram"] as const).map((c) => (
                <button
                  key={c}
                  onClick={() =>
                    setBalanceAdjustForm({ ...balanceAdjustForm, currency: c })
                  }
                  style={{
                    flex: 1,
                    background:
                      balanceAdjustForm.currency === c
                        ? "#0FD37C"
                        : "rgba(255,255,255,0.04)",
                    color:
                      balanceAdjustForm.currency === c ? "#000" : "#8A8F98",
                    border: "none",
                    borderRadius: 8,
                    padding: "6px 0",
                    fontSize: 11,
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
              {(["add", "deduct", "correct"] as const).map((t) => (
                <button
                  key={t}
                  onClick={() =>
                    setBalanceAdjustForm({ ...balanceAdjustForm, type: t })
                  }
                  style={{
                    flex: 1,
                    background:
                      balanceAdjustForm.type === t
                        ? "#11ABEC"
                        : "rgba(255,255,255,0.04)",
                    color: balanceAdjustForm.type === t ? "#fff" : "#8A8F98",
                    border: "none",
                    borderRadius: 8,
                    padding: "6px 0",
                    fontSize: 11,
      ...
      {resetModal && (
        ...
        >
          ...
          >
            ...
            >
              تأكيد تصفير رصيد{" "}
              {resetModal === "coins" ? "الـ Rush/Coins" : "الـ GRAM"}
            </div>
            <p
              style={{
                fontSize: 11,
            ...
            >
              سيتم تصفير الرصيد لجميع المستخدمين في قاعدة البيانات فورياً. هذا
